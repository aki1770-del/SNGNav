...
class _WeatherMarker extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    // The state is carried on LUMINANCE, shape and size, and colour last.
    //
    // Luminance first, because it is what survives glare, a sun-washed panel,
    // dusk, peripheral vision and colour-vision deficiency. The two dangerous
    // confusions are carried on it: "there is ice" is the darkest mark and "the
    // road is fine" is the lightest, and "we did not measure this road" sits
    // between them. Mistaking hazardous for unknown is the safe direction, and
    // those two are also apart on shape and size.
    //
    // The previous palette (a red #B71C1C triangle, an amber #FFC107 disc with a
    // dark ring, a blue #0D47A1 dot) was rendered on the offline basemap and
    // measured. With colour removed, the ice warning and the clear road were the
    // same grey at their centres. With fine detail lost, the unknown disc was
    // the quietest mark on the map, because its amber fill sits at the map's own
    // luminance and only a thin ring carried it.
    //
    // Salience is ordered hazardous > unknown > clear. "We did not measure this
    // road" must never be quieter than "the road is fine".
    //
    // Glyph size and colour count: a large light glyph in a dark mark makes its
    // centre grey, and the centre is what a blurred glance keeps.
    switch (verdict) {
      case SafetyVerdict.hazardous:
        // Triangle: the learned warning shape. Darkest and largest.
        return const SizedBox(
          ...
          child: DecoratedBox(
            ...
            child: Align(
              ...
              child: Icon(Icons.ac_unit, color: Color(0xFFFFFFFF), size: 18),
            ),
          ),
        );
      case SafetyVerdict.unknown:
        // NOT a green light, and not a quiet one either: a solid mid-dark disc
        // that holds against a pale basemap on its fill, and a broad white ring
        // that holds against dark labels and roads, where a light clear mark
        // would otherwise be the brighter change.
        return const Center(
          child: _RingedDot(
            fill: Color(0xFFEF6C00),
      ...
      case SafetyVerdict.notHazardous:
        // Smallest, quietest and the only light mark. An assessed-and-clear
        // road asks nothing of her, and cannot be mistaken for a dark warning.
        return const Center(
          child: _RingedDot(
            fill: Color(0xFFFFFFFF),
            ring: Color(0xFF0B3D91),
            ringWidth: 2,
            size: 30,
            icon: Icons.cloud,
            iconColor: Color(0xFF0B3D91),
            iconSize: 14,
          ),
        );
    }
