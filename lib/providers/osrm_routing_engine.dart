...
///
/// 95–133× faster than Valhalla (benchmarked):
...
/// At 60 km/h, Valhalla's 464 ms = 7.7 m of stale route display.
///
...
import 'valhalla_routing_engine.dart'; // RoutingException
...
class OsrmRoutingEngine implements RoutingEngine {
  ...
  Future<bool> isAvailable() async {
    try {
      // OSRM health check: nearest service on a known coordinate.
  ...
  /// Map OSRM maneuver type+modifier to engine-agnostic type strings.
  ...
  List<LatLng> _decodePolyline5(String encoded) {
    ...
    while (index < encoded.length) {
      ...
      points.add(LatLng(lat / 1e5, lng / 1e5)); // precision 5, not 6
    }
