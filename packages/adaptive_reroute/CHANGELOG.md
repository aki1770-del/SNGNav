...
The published 0.2.1 `pubspec.yaml` still carried development-only
`dependency_overrides` pointing at `../route_condition_forecast`, `../driving_weather` and `../routing_engine` -- folders that exist only in this
...
code 66: "depends on routing_engine from path which doesn't exist". pub does not remove
`dependency_overrides` when a package is published.

Apps that depend on `adaptive_reroute` were not affected: a dependency's overrides never
apply to the app that uses it. Checked: an app depending on `adaptive_reroute: 0.2.1`
resolves normally.
