...
void main() {
  group('canAssertNoAdvisory — the one gate on claiming silence', () {
    test('genuinely clear: all sources answered, empty list → CAN assert', () async {
      ...
      await agg.init();
      final r = await agg.fetchActiveAdvisoriesAtPoint(latitude: 0, longitude: 0);

      expect(r.advisories, isEmpty);
      expect(r.canAssertNoAdvisory, isTrue,
          reason: 'every source answered; an empty list here is a real all-clear');
      expect(r.isUnavailable, isFalse);
    });

    test(
        'the defect this ships to fix: EVERY source down → empty list, but '
      ...
      expect(r.canAssertNoAdvisory, isFalse,
          reason: 'no source was successfully read — this is an outage, not clear');
      expect(r.isUnavailable, isTrue);
    });

    test('partial outage: some seen → act on them, but cannot claim silence',
        () async {
      final agg = AdvisoryAggregator(
        providers: <AdvisoryProvider>[
          _StubProvider(
            source: AdvisorySource.nwsUnitedStates,
            advisories: <Advisory>[_adv(AdvisorySource.nwsUnitedStates, 'snow')],
          ),
          _StubProvider(
            source: AdvisorySource.jmaJapan,
      ...
      expect(r.canAssertNoAdvisory, isFalse,
          reason: 'the warning we are missing may be in the source that is down');
      expect(r.isUnavailable, isFalse);
    });
  });

  group('typed failure reason — a sentence the driver can read', () {
    test('timeout / parse / init classified; unknown stays unclassified',
        () async {
      final agg = AdvisoryAggregator(
        providers: <AdvisoryProvider>[
          _StubProvider(
      ...
      expect(bySource[AdvisorySource.jmaJapan],
          AdvisoryUnavailableReason.timedOut);
      expect(bySource[AdvisorySource.nwsUnitedStates],
      ...
      expect(bySource[AdvisorySource.metNorway],
          AdvisoryUnavailableReason.unclassified);
      final met = r.providerErrors
          .firstWhere((e) => e.source == AdvisorySource.metNorway);
      expect(met.cause, isA<StateError>());
    });
  });

  group('fold — handle every case or do not compile', () {
    test('routes complete / partial / unavailable to the right branch', () async {
      ...
      final clear = AdvisoryAggregator(providers: <AdvisoryProvider>[
        _StubProvider(source: AdvisorySource.nwsUnitedStates),
      ]);
      await clear.init();
      expect(banner(await clear.fetchActiveAdvisoriesAtPoint(latitude: 0, longitude: 0)),
          'NO_ADVISORY');

      final down = AdvisoryAggregator(providers: <AdvisoryProvider>[
        _StubProvider(
          source: AdvisorySource.jmaJapan,
      ...
      await down.init();
      expect(banner(await down.fetchActiveAdvisoriesAtPoint(latitude: 0, longitude: 0)),
          'FEED_DOWN');

      final partial = AdvisoryAggregator(providers: <AdvisoryProvider>[
        _StubProvider(
          source: AdvisorySource.nwsUnitedStates,
      ...
      await partial.init();
      expect(banner(await partial.fetchActiveAdvisoriesAtPoint(latitude: 0, longitude: 0)),
          'PARTIAL');
    });
  });
  ...
  group('requireCompleteLookup — opt-in loud stop with a way forward', () {
    ...
    test('throws a lamp — names the source, the reason, and the way forward',
        () async {
      final agg = AdvisoryAggregator(providers: <AdvisoryProvider>[
        _StubProvider(
          source: AdvisorySource.jmaJapan,
      ...
      expect(
        ...
        throwsA(
          isA<AdvisoryLookupIncompleteException>()
              ...
              .having((e) => e.message, 'says the reason', contains('timedOut'))
