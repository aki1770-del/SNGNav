...
//
// Adaptations from the 0.0.8 originals — only where 0.1.0's API genuinely
// changed, each justified:
//   (1) `fetchActiveAdvisoriesAtPoint` now returns the sealed `AdvisoryLookup`
//       (the 0.1.0 breaking change itself), so aggregator-returned results
//       read `r.seen` / `r.unreachable` / `r.failures` where 0.0.8 read
//       `r.advisories` / `r.providerErrors`. `seen` and `unreachable` are the
//       names 0.0.8 itself introduced "so the migration is a rename you can
//       make today" — this file is that promise, kept.
//   (2) The per-failure record at the lookup surface is `AdvisorySourceFailure`
//       (reason + cause; no string `message`), so the typed-reason test reads
//       `failures` instead of `providerErrors`. The exception contract is
//       UNCHANGED: `AdvisoryLookupIncompleteException.unreachable` still
//       carries `AdvisoryProviderError` values, message included.
//   (3) The 0.0.8 exception message pointed forward to "0.1.0"; in 0.1.0 the
//       forward pointer is the sealed type itself, so the expectation is
//       `contains('AdvisoryLookup')` instead of `contains('0.1.0')`.
...
void main() {
  group('canAssertNoAdvisory — the one gate on claiming silence', () {
    test(
      ...
      () async {
        ...
        expect(
          ...
          isTrue,
          reason:
              'every source answered; an empty list here is a real '
              'all-clear',
        );
    ...
    test('the defect this ships to fix: EVERY source down → empty list, but '
      ...
      expect(
        ...
        isFalse,
        reason:
            'no source was successfully read — this is an outage, not '
            'clear',
      );
    ...
    test(
      ...
      () async {
        ...
        expect(
          ...
          isFalse,
          reason:
              'the warning we are missing may be in the source that is '
              'down',
        );
        expect(r.isUnavailable, isFalse);
      },
  ...
  group('typed failure reason — a sentence the driver can read', () {
    test(
      ...
      () async {
        ...
        final bySource = {for (final e in r.failures) e.source: e.reason};
        expect(
        ...
        // original preserved in `cause`. (A bare StateError is NOT assumed to
        // mean "not initialised" — only the typed AdvisoryProviderInitException
        // earns that reason.)
        expect(
          bySource[AdvisorySource.metNorway],
          AdvisoryUnavailableReason.unclassified,
        );
        final met = r.unreachable.firstWhere(
  ...
  group('fold — handle every case or do not compile', () {
    test(
      ...
      () async {
        String banner(AdvisoryLookup r) => r.fold(
  ...
  group('requireCompleteLookup — opt-in loud stop with a way forward', () {
    ...
    test('the exception keeps the 0.0.8 contract: unreachable entries are '
        'AdvisoryProviderError with message + typed reason + cause', () async {
      final agg = AdvisoryAggregator(
        providers: <AdvisoryProvider>[
          _StubProvider(
            source: AdvisorySource.jmaJapan,
      ...
      try {
        r.requireCompleteLookup();
        fail('expected AdvisoryLookupIncompleteException');
      } on AdvisoryLookupIncompleteException catch (e) {
        final err = e.unreachable.single;
        expect(err, isA<AdvisoryProviderError>());
        expect(err.source, AdvisorySource.jmaJapan);
        expect(err.reason, AdvisoryUnavailableReason.timedOut);
        expect(err.message, contains('JMA down'));
        expect(err.cause, isA<TimeoutException>());
      }
    });
  });
  ...
  group('hand-built result keeps 0.0.7 source-compatibility', () {
    ...
    test('the full 0.0.8 result surface survives on the hand-built shape '
        '(fold + requireCompleteLookup + getters)', () {
      const err = AdvisoryProviderError(
        source: AdvisorySource.jmaJapan,
      ...
      const r = AdvisoryAggregateResult(
        advisories: <Advisory>[],
        providerErrors: <AdvisoryProviderError>[err],
        sourcesQueried: 2,
      );
      expect(r.seen, isEmpty);
      expect(r.unreachable, equals(const <AdvisoryProviderError>[err]));
      expect(r.canAssertNoAdvisory, isFalse);
      expect(r.isUnavailable, isFalse, reason: '1 of 2 sources answered');
      expect(
        r.fold(
          complete: (a) => 'COMPLETE',
          partial: (seen, down) => 'PARTIAL',
