...
const String _akitaPrefectureAtomFeed =
...
const String _akitaReportWithOoyukiKeihou =
    '''<?xml version="1.0" encoding="UTF-8"?>
...
const String _naganoReportWithStrongWindOnly =
    '''<?xml version="1.0" encoding="UTF-8"?>
...
void main() {
  ...
  group('JmaAdvisoryProvider — fetch behavior', () {
    ...
    test('fetch returns empty list when atom feed has no winter-advisory '
        'entries for the caller prefecture', () async {
...
<feed xmlns="http://www.w3.org/2005/Atom">
  <entry>
    ...
    test('fetch with mocked Akita 大雪警報 returns one Advisory with '
        'eventClass = "大雪警報" verbatim', () async {
      final mock = MockClient((req) async {
      ...
      expect(advisories.first.severity, equals(AdvisorySeverity.severe));
      expect(advisories.first.areaDescription, equals('秋田中央'));
      expect(advisories[1].eventClass, equals('暴風雪注意報'));
      expect(advisories[1].severity, equals(AdvisorySeverity.moderate));
    });

    test('fetch filters out non-snow event classes (強風注意報) even when '
        'present in the prefecture report', () async {
      final mock = MockClient((req) async {
        if (req.url.toString().endsWith('feed.xml')) {
          return _utf8Response(_akitaPrefectureAtomFeed, 200);
        }
  ...
  group('parseJmaReportXml', () {
    ...
    test('preserves Headline text verbatim', () {
      final records = parseJmaReportXml(_akitaReportWithOoyukiKeihou);
      expect(records.first.headline, equals('秋田県では、１５日夜のはじめ頃まで大雪に警戒してください。'));
    });
  ...
  group('mapJmaForecastToAdvisory', () {
    ...
    test('eventClass preserves JMA event-name verbatim (Article 17 β)', () {
      ...
      expect(mapJmaForecastToAdvisory(record).eventClass, equals('暴風雪警報'));
    });
    ...
    test('kJmaSnowAdvisoryEventNames includes the 4-minimum + 着雪', () {
      for (final name in const ['大雪警報', '大雪注意報', '暴風雪警報', '暴風雪注意報']) {
