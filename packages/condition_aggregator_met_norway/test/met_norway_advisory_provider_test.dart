...
void main() {
  group('mapLocationForecastResponseToAdvisory', () {
    test('maps freezing precipitation slice to severe Advisory with metNorway source', () {
      ...
      final advisory = mapLocationForecastResponseToAdvisory(response: response);
    ...
    test('maps freezing precipitation with heavy precipitation rate to extreme', () {
      final response = _baseResponse(
        time: '2026-05-24T06:00:00Z',
        airTemperature: -3.0,
    ...
    test('maps subzero forecast without precipitation to moderate severity', () {
      final response = _baseResponse(
        time: '2026-05-24T06:00:00Z',
        airTemperature: -5.0,
    ...
    test('returns null when above-freezing AND no heavy precipitation (no advisory)', () {
      final response = _baseResponse(
        time: '2026-05-24T06:00:00Z',
        airTemperature: 15.8,
        precipitationNext1h: 0.0,
    ...
    test('returns null when timeseries is absent or empty', () {
      ...
      expect(
        mapLocationForecastResponseToAdvisory(response: response),
        isNull,
      );
    });
  });
  ...
  group('MetNorwayAdvisoryProvider', () {
    test('source is AdvisorySource.metNorway (parent enum carries the member)',
        () {
    ...
    test('init() succeeds with default User-Agent + non-negative thresholds',
        () async {
      final provider = MetNorwayAdvisoryProvider();
      try {
        await provider.init();
    ...
    test('init() throws MetNorwayConfigurationException on empty User-Agent',
        () async {
    ...
    test(
        'fetchActiveAdvisoriesAtPoint returns an Advisory when MET Norway '
    ...
    test(
        'fetchActiveAdvisoriesAtPoint returns an empty list when no '
    ...
    test('fetchActiveAdvisoriesAtPoint throws MetNorwayHttpException on non-200',
        () async {
      final mock = MockClient((http.Request request) async {
    ...
    test('coordinates are truncated to four decimal places before request',
        () async {
      Uri? observedUri;
      final mock = MockClient((http.Request request) async {
