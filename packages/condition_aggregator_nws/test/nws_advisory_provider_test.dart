...
void main() {
  group('mapWinterAlertToAdvisory severity gradient', () {
    ...
    test('all five severity levels map to interface enum', () {
      expect(
        mapWinterAlertToAdvisory(
          alertWithSeverity(AlertSeverity.minor),
        ).severity,
        AdvisorySeverity.minor,
      );
      expect(
        mapWinterAlertToAdvisory(
          alertWithSeverity(AlertSeverity.moderate),
        ).severity,
        AdvisorySeverity.moderate,
      );
      expect(
        mapWinterAlertToAdvisory(
          alertWithSeverity(AlertSeverity.severe),
        ).severity,
        AdvisorySeverity.severe,
      );
      expect(
        mapWinterAlertToAdvisory(
          alertWithSeverity(AlertSeverity.extreme),
        ).severity,
        AdvisorySeverity.extreme,
      );
      expect(
        mapWinterAlertToAdvisory(
          alertWithSeverity(AlertSeverity.unknown),
        ).severity,
    ...
    test('high-impact identification preserved through mapping', () {
      final extreme = mapWinterAlertToAdvisory(
        alertWithSeverity(AlertSeverity.extreme),
      );
      final moderate = mapWinterAlertToAdvisory(
        alertWithSeverity(AlertSeverity.moderate),
      );
  ...
  group('mapWinterAlertToAdvisory area + verbatim wording', () {
    test('areaDesc preserved verbatim', () {
      ...
      expect(mapped.headline, 'Blizzard Warning issued by NWS Grand Forks ND');
  ...
  group('mapWinterAlertToAdvisory certainty + urgency mapping', () {
    test('certainty levels map directly', () {
      ...
      expect(
        mapWinterAlertToAdvisory(mk(AlertCertainty.observed)).certainty,
        AdvisoryCertainty.observed,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertCertainty.likely)).certainty,
        AdvisoryCertainty.likely,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertCertainty.possible)).certainty,
        AdvisoryCertainty.possible,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertCertainty.unlikely)).certainty,
        AdvisoryCertainty.unlikely,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertCertainty.unknown)).certainty,
    ...
    test('urgency levels map directly', () {
      ...
      expect(
        mapWinterAlertToAdvisory(mk(AlertUrgency.immediate)).urgency,
        AdvisoryUrgency.immediate,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertUrgency.expected)).urgency,
        AdvisoryUrgency.expected,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertUrgency.future)).urgency,
        AdvisoryUrgency.future,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertUrgency.past)).urgency,
        AdvisoryUrgency.past,
      );
      expect(
        mapWinterAlertToAdvisory(mk(AlertUrgency.unknown)).urgency,
