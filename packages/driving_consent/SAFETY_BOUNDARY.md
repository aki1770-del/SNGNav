...
- **Per-purpose, not blanket** (`ConsentPurpose` enum): the package decomposes consent into purpose-class buckets (fleetLocation / weatherSharing / vehicleDiagnostics / etc.). The driver grants fleet-location sharing without granting diagnostic-data sharing. Blanket consent is forbidden by the API surface itself.
...
- **Pluggable storage interface** (`ConsentService` abstract): the package does not lock the integrator into a specific persistence backend. The in-memory implementation is for tests; production integrators implement their own (encrypted at rest, jurisdiction-appropriate retention, etc.). Storage-class concerns are integrator-responsibility.
These five disciplines collectively form the package's SOTIF-class advisory-honesty posture at the privacy-class gate boundary.
...
**What HER experiences when this package fires**: *consent surface that respects every driver's right to choose what data flows from her vehicle to fleet aggregation, with revocability per session.* When `driving_consent` fires through an integrator HMI:
- HER sees a per-purpose disclosure (e.g. "Share vehicle position with fleet aggregation server for snow-road hazard detection?") — not a blanket "agree to everything" toggle.
- HER chooses *grant* or *deny* per purpose; the choice is recorded with purpose, jurisdiction, and timestamp.
- HER can revoke a previously-granted purpose at any session boundary; the integrator's downstream pipeline observes the revocation at its next gate-check and stops the flow.
- HER never has data flow under UNKNOWN status — the silent default is DENIED, not GRANTED.
...
**Driver-facing-loom field**: this section is the canonical driver-facing-loom declaration for `driving_consent` 0.3.0. Subsequent versions update this field on material changes to the privacy-class surface (new purpose classes, new jurisdiction classes, revocation-protocol changes, etc.).
...
- `lib/src/consent_record.dart` (`ConsentStatus` / `ConsentPurpose` / `Jurisdiction` enums; `ConsentRecord` value-object)
...
- Composition: `driving_consent` upstream gate → integrator's data-egress pipeline → fleet aggregation or no aggregation per consent state.
