...
- **Ice-risk surface**: `WeatherCondition` exposes an ice-risk-class field; the integrator decides whether the field's value triggers a hazard advisory at the integrator's HMI surface. The package does not emit an alert; it exposes the substrate.
- **HTTP-class failure surface**: network failures surface as exceptions or null returns; the integrator's retry/backoff policy is the integrator's responsibility (per the `noaa_nws_adapter` 0.0.2 retry/backoff pattern; consider mirroring at the integrator layer for `driving_weather` too).
...
- HER sees the weather state surfaced at her vehicle's location: precipitation type, intensity, wind speed, visibility distance, ice-risk flag.
...
**Sakichi reading**: the loom is *the upstream observer that surfaces the world's state honestly, never asserts more than it observes, and yields the rendering choice to the integrator's profile-aware HMI*. The fusion-with-road-surface composition (`condition_aggregator`) ensures the driver sees one converged decision substrate; the driver does not have to integrate three separate alert streams in HER head while operating the vehicle.
...
**Driver-facing-loom field**: this section is the canonical driver-facing-loom declaration for `driving_weather` 0.3.0. Subsequent versions update this field on material changes to the weather-class surface (new providers, new fields, new fusion patterns, etc.).
...
- `lib/src/weather_condition.dart` (`PrecipitationType` / `PrecipitationIntensity` / `WeatherCondition` value-object)
...
- `lib/src/simulated_weather_provider.dart` (test-class deterministic provider)
