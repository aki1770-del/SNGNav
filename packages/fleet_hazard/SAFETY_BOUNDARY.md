...
**Recency correction (0.5.1)**: up to and including 0.5.0 the "last 30 min" in the line above was **not enforced by any code in this package**. `FleetReport.isRecent` existed but nothing called it, and `HazardAggregator.aggregate()` applied no age filter — every report counted, however old. An integrator rendering that string against `aggregate()` was making a claim the API could not support. As of 0.5.1 the window is real, but **only when the integrator asks for it**: `HazardAggregator.aggregateWithProvenance(reports, maxAge: const Duration(minutes: 30))` consults `FleetReport.isRecent`, excludes older reports from clustering, and counts them in `FleetAggregate.staleCount`. `aggregate()` still applies no recency filter, by design — it is unchanged for existing callers.

**Absence-is-not-all-clear (0.5.1)**: `aggregate()` returns an empty `List<HazardZone>` in three distinct worlds — no reports at all, reports that all say the road is clear, and reports that all say `RoadCondition.unknown` (driven, unreadable). It cannot distinguish them, so **an integrator must never render its empty result to HER as a clear road**. `aggregateWithProvenance()` returns `FleetAggregate` with `isSilent` / `isMeasuredClear` / `isAllUnknown` so the HMI can say *"nobody has reported this road"* where that is the truth. Absence must not be rendered with the same calm as a measured all-clear; on an unreported road that calm is what puts HER on the ice.
