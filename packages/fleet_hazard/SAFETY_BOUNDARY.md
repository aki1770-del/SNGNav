...
**PHIL-001 boundary anchor**: `FleetReport` carries `vehicleId` per README.md L36; the boundary holds at aggregate-class consumption (vehicleId is opaque-token-class for cluster attribution; never PII; never mapped to driver identity inside this package's substrate). Integrator implementations MUST preserve this boundary at the `FleetProvider` source side — feeding identified-driver telemetry into this package would breach the boundary at the integration scope.
...
- map clusters where snow / ice was reported by vehicles ahead of HER on the route — not raw per-vehicle dots (which would be noise) but clustered zones at appropriate radius
...
**Driver-facing-loom field**: this section is the canonical D-VGC189-1 declaration for `fleet_hazard` 0.3.0. Subsequent versions update this field on material changes to the driver-experience surface (e.g. confidence-rendering shape change → field update; internal cluster-radius parameter tuning → no field update).
