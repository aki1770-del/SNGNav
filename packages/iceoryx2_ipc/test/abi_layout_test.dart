...
void main() {
  test('sngnav_road_friction_t: Dart and C layouts agree, field by field', () {
    final pkg = Directory.current.path;

    final tool = File('$pkg/../driving_conditions/tool/abi_layout_check.dart');
    if (!tool.existsSync()) {
    ...
    final r = Process.runSync('dart', [
      ...
      tool.path,
      '--c', header.path,
      '--dart', bindingFile.path,
      '--pair', 'sngnav_road_friction_t=$dartClass',
    ]);
    ...
    expect(output, contains('size=24 align=8'),
    ...
    expect(output, contains('agrees on size, alignment and every field offset'));
