...
void main() {
  test('a sample written by the C publisher arrives and classifies in Dart',
      () async {
    ...
    final libLine = env
        ...
        .firstOrNull;
    if (libLine == null) {
      _unverified('ICEORYX2_LIB is not recorded in native/.iceoryx2.env',
    ...
    expect(
      ...
      {0, 1, 2, 3},
      reason: 'did not observe all four phases of the scripted series within '
      ...
      switch (phase) {
        ...
        case 3:
          ...
          expect(
            ...
            isFalse,
            reason: 'seq ${s.sequence}: the -999.0 sentinel leaked through '
                'instead of being suppressed by quality == 0 — absence is '
                'riding the measurement scale',
          );
          expect(s.reading.rawValue, isNull,
    ...
    for (var i = 1; i < seqs.length; i++) {
      expect(
        ...
        reason: 'sequence went backwards: $seqs — a stale sample was delivered '
            'as a current one',
      );
    }
