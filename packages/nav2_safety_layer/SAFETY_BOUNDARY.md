...
| **D-07** | **D** | Advisory burst consumes the throttle cap, then a nav2 `STOP` arrives | Severity is **hardcoded** `AlertSeverity.warning` (`nav2_safety_layer.dart:47`, `:59`); the core critical-bypass is **never engaged** | **The `STOP` is silently discarded.** Measured: cap 2/min, 5×`LIMIT`, then `STOP` → delivered = **false**. | **fail-safe** — `STOP` must bypass. Currently **the most severe upstream signal is the most droppable**. |
...
| **AoU-5** | **You supply your own escalation path for `STOP`.** This package will drop it under load (D-07). Do not route a reflexive-stop signal through this package alone. |
| **AoU-6** | **You accept that the advisory text names road ice** regardless of what the monitor actually reported (PI-06), and you decide whether that text may be shown to a driver. We would not show it. |
