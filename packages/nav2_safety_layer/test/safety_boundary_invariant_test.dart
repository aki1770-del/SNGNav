...
void main() {
  ...
  // ---------------------------------------------------------------------------
  group(
    'BI-1 — no transport dependency (SAFETY_BOUNDARY §7, "driver always drives")',
    () {
    ...
    () {
      ...
      test('PI-04: length mismatch is still absorbed as a shorter loop', () {
        ...
        expect(
          ...
          isEmpty,
          reason:
              'PI-04 pinned. 5 polygons against 2 detections is proof the '
      ...
      test(
        ...
        () async {
          ...
          layer.onMonitorState(
            Nav2CollisionMonitorState.fromJson({'polygon_name': 'front'}),
          ); // PI-01
          layer.onMonitorState(
            Nav2CollisionMonitorState.fromJson({
              'action_type': 5,
              'polygon_name': 'front',
            }),
          ); // PI-02
          layer.onDetectorState(
            Nav2CollisionDetectorState.fromJson({
              'polygons': ['front', 'rear'],
            }),
          ); // PI-03
          await settle();
  ...
  group('BI-6 — PI-05: four upstream action classes collapse to one string', () {
    test('STOP and LIMIT are byte-identical at the HMI', () {
      ...
      final limit = Nav2SafetyMapper.toAdvisory(
        const Nav2CollisionMonitorState(
          actionType: Nav2CollisionAction.limit,
          polygonName: 'front',
        ),
        p,
      )!;
      expect(
        ...
        equals(limit.action),
        reason:
            'PI-05 pinned. nav2 STOP ("stop the robot") and LIMIT ("limit '
    ...
    test('all four non-doNothing actions produce one distinct string', () {
      ...
      for (final a in Nav2CollisionAction.values) {
        ...
        texts.add(
          Nav2SafetyMapper.toAdvisory(
            Nav2CollisionMonitorState(actionType: a, polygonName: 'p'),
            p,
          )!.action,
        );
      }
      expect(
        ...
        hasLength(1),
        reason:
            'PI-05 pinned: the mapper is information-destroying by design '
  ...
  group('BI-7 — PI-06: obstacle events must not assert a road-surface cause', () {
    test('an obstacle no longer yields a fabricated frozen-surface claim', () {
      ...
      expect(
        ...
        reason:
            'PI-06 corrected in the working tree (nav2_safety_mapper.dart). '
      ...
      expect(
        ...
        isNot(contains('凍結')),
        reason:
            'No frozen-surface claim may be synthesised from a message '
      ...
      expect(
        ...
        reason:
            'PI-06 sharpest form, now closed in-tree: the advisory must '
            'never counter-instruct the manoeuvre an upstream STOP demands.',
      );
    });

    test(
      'the residual is real and is pinned: no obstacle vocabulary exists',
      () {
        final ex = Nav2SafetyMapper.toAdvisory(
          const Nav2CollisionMonitorState(
            actionType: Nav2CollisionAction.stop,
        ...
        expect(
          ...
          contains('路面状況不明'),
          reason:
              'RESIDUAL, recorded not closed. `.unknown` is honest and still '
  ...
  group('BI-8 — D-08: the monitor path does not relay polygon_name', () {
    test('polygon_name is absent from monitor-path advisory text', () {
      final ex = Nav2SafetyMapper.toAdvisory(
        const Nav2CollisionMonitorState(
          actionType: Nav2CollisionAction.stop,
          polygonName: 'REAR_LEFT_ZONE',
        ),
        DriverProfile.snowZoneExperienced,
      )!;
      expect(
        ex.action.contains('REAR_LEFT_ZONE'),
        isFalse,
        reason:
            'D-08 pinned. nav2_safety_mapper.dart:7-10 states polygon_name is '
    ...
    test(
      'the detector path DOES relay polygon names — the divergence is path-specific',
      () async {
        final layer = Nav2SafetyLayer(
          profile: DriverProfile.snowZoneExperienced,
          throttle: AlertDensityThrottle(alertsPerMinuteCap: 60),
        ...
        expect(
          got.single,
          contains('REAR_LEFT_ZONE'),
          reason:
              'nav2_safety_layer.dart:64-65 relays on the detector path only. '
  ...
  group('BI-9 — D-07 REPAIRED: a nav2 STOP survives advisory load', () {
    test('the core critical-bypass works, and this package NOW asks for it', () {
      ...
      expect(
        t.shouldFire(now, AlertSeverity.warning),
        isFalse,
        reason: 'cap consumed',
      );
      expect(
        t.shouldFire(now, AlertSeverity.critical),
        isTrue,
        reason:
            '⚑ CORRECTED 2026-08-21. navigation_safety_core '
    ...
    test(
      'a burst of LIMIT consumes the cap and the following STOP STILL reaches the driver',
      () async {
        final layer = Nav2SafetyLayer(
          profile: DriverProfile.snowZoneExperienced,
        ...
        expect(
          ...
          equals(afterBurst + 1),
          reason:
              '⚑ D-07 REPAIRED 2026-08-21 — this assertion is INVERTED from a pin '
  ...
  // ---------------------------------------------------------------------------
  group(
    'BI-10 — the boundary record exists and is reachable from the package',
    () {
      test('SAFETY_BOUNDARY.md is present beside pubspec.yaml', () {
        final dir = _pubspec().parent;
        final rec = File(
          '${dir.path}${Platform.pathSeparator}SAFETY_BOUNDARY.md',
        );
        expect(
          rec.existsSync(),
          isTrue,
          reason:
              'The package named `safety` was the one package in this catalog '
        ...
        for (final id in [
          ...
          expect(
            ...
            contains(id),
            reason:
                'Every ID asserted in this oracle must resolve to a row in the '
