...
- Widen `latlong2` from `^0.9.1` to `>=0.9.1 <0.11.0`.
  ...
  gratuitous. Verified on `latlong2 0.10.1`: analyze clean, **111/111 tests pass**.

## 0.9.5

Adds an optional `conditionCleared` parameter to `SafetyAlertDismissed`
(default `false`): the hazard owner — the component that raised an alert —
can now clear it when the condition passes, including non-dismissible
alerts, with `SafetyAlertDismissed(conditionCleared: true)`.

In 0.9.4 there was no way to clear a non-dismissible alert. Plain
`SafetyAlertDismissed` refuses non-dismissible alerts by design,
`RerouteCompleted` preserves them by design, and no other event touches the
alert fields — so once a non-dismissible alert fired, the overlay stayed
modal for the process lifetime even after the hazard had passed. The shipped
example deadlocked on one tap of its "Critical" button: the demo controls
sit under `SafetyOverlay`'s modal barrier, which swallows their taps while
an alert is active.

The distinction the flag keeps intact: `SafetyAlertDismissed()` without the
flag is the driver waving an alert away, and it is still refused for
non-dismissible alerts; `conditionCleared: true` is the condition pipeline
reporting that the hazard no longer holds. Only the component that raised or
monitors the condition should pass it — do not wire it to a driver-facing
dismiss control.

The example now keeps a condition-clear control above the overlay barrier
(standing in for the hazard-owner pipeline), so the demo can be exercised
past a non-dismissible alert.

No new event types: `NavigationEvent` is sealed, so code that switches
exhaustively over its subtypes compiles unchanged. The only API change is
the new optional named parameter (and its value now appearing in
`SafetyAlertDismissed.props`); existing calls behave exactly as in 0.9.4.
