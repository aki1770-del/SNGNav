...
| S-051 | Low-vision accessibility contrast | `contrastRatio` (0.0–1.0; WCAG 2.2 AA asks for a contrast ratio of at least 4.5:1, on its 1–21 scale; no mapping from that scale to 0.0–1.0 is defined here, so the PR adding the field defines the mapping, its direction and what 0.0 means, and tests it) | hard |
