...
- [driving_weather](https://pub.dev/packages/driving_weather) - Weather condition model for driving
- [driving_consent](https://pub.dev/packages/driving_consent) - Privacy consent with Jidoka semantics
- [fleet_hazard](https://pub.dev/packages/fleet_hazard) - Fleet telemetry hazard model and clustering
- `driving_conditions` — Pure Dart computation models: road surface classification, visibility degradation, precipitation config, Monte Carlo safety score simulation (imports `SafetyScore` from this package's `_core` barrel)
