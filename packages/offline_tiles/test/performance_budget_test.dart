...
void main() {
  group('PerformanceBudgetConfig', () {
    test('default config: 16ms baseline budget, 0.75 warning ratio', () {
      ...
      expect(config.frameBudget, PerformanceBudgetConfig.baselineFrameBudget);
    ...
    test('warningRatio out of range fails assert', () {
      expect(
        () => PerformanceBudgetConfig(warningRatio: 0.0),
        throwsA(isA<AssertionError>()),
      );
      expect(
        () => PerformanceBudgetConfig(warningRatio: 1.0),
        throwsA(isA<AssertionError>()),
      );
    });
    ...
    test('sustainedFrames < 1 fails assert', () {
      expect(
        () => PerformanceBudgetConfig(sustainedFrames: 0),
        throwsA(isA<AssertionError>()),
      );
    });
  });
  ...
  group('PerformanceBudget', () {
    ...
    test('BudgetExhausted fires only after sustainedFrames consecutive '
    ...
    test(
      'non-over-budget frame resets the consecutive-over-budget counter',
      () async {
        final tracker = PerformanceBudget(
          config: const PerformanceBudgetConfig(sustainedFrames: 3),
        );
