...
void _closeTo(String? actual, String expected, int toleranceMin, String why) {
  ...
  expect(diff <= toleranceMin, isTrue,
...
const _hazardFragments = <String>[
  'ice', 'freez', 'frost', 'black ice', 'refro', 'slick', 'slush',
  'snow', 'packed', 'whiteout', 'precip', 'fog',
  '凍', '氷', '霜', '滑', '雪', '圧雪', '降水', 'ホワイトアウト',
];

void _assertNoFabricatedHazard(String chip) {
  for (final f in _hazardFragments) {
    expect(chip.toLowerCase().contains(f.toLowerCase()), isFalse,
        reason: 'daylight chip must not assert a hazard; found "$f" in: $chip');
  }
}

// Geographies (lat N+, lon E+, civil offset).
const _akita = TripGeo(latitude: 39.72, longitude: 140.10, utcOffset: Duration(hours: 9));
const _tokyo = TripGeo(latitude: 35.69, longitude: 139.69, utcOffset: Duration(hours: 9));
const _london = TripGeo(latitude: 51.51, longitude: -0.13, utcOffset: Duration());
const _anchorage =
    TripGeo(latitude: 61.22, longitude: -149.90, utcOffset: Duration(hours: -9));
const _tromso = TripGeo(latitude: 69.65, longitude: 18.96, utcOffset: Duration(hours: 1));
...
const _highArctic =
    TripGeo(latitude: 80.0, longitude: 15.0, utcOffset: Duration(hours: 1));
...
const _fastDriver =
    DriverProfileSpec(profileTag: 'test', reactionTimeSeconds: 1.0);

void main() {
  group('NOAA solar anchors (offline astronomy)', () {
    test('Akita 2026-01-15 — real external anchor (sunrise 06:59 / sunset 16:38, ±2)', () {
    ...
    test('Tokyo 2026-01-15 — sunrise ~06:50 / sunset ~16:51 (real external anchor, ±2)', () {
    ...
    test('London 2026-01-15 — sunrise ~07:58 / sunset ~16:21', () {
      ...
      expect(_min(d.sunriseHHMM!), inInclusiveRange(_min('07:56'), _min('08:01')),
          reason: 'London sunrise: ${d.sunriseHHMM}');
      expect(_min(d.sunsetHHMM!), inInclusiveRange(_min('16:18'), _min('16:24')),
          reason: 'London sunset: ${d.sunsetHHMM}');
    });

    test('Tromso 2025-12-21 — POLAR NIGHT: sunrise & sunset are NULL (never fabricated)',
        () {
  ...
  group('midnight-wrap (event minutes > 1440 roll across the UTC day)', () {
    ...
    test('Anchorage sunset — event UTC is the NEXT day (minutes > 1440), local ~16:xx', () {
      ...
      expect(_min(d.sunsetHHMM!), inInclusiveRange(_min('16:05'), _min('16:30')),
  ...
  group('wall-clock semantics (correction c: isUtc-independent)', () {
    test('same wall-clock fields => same result whether DateTime is local or utc', () {
      final local = evaluateDaylight(DateTime(2026, 1, 15, 6, 30), _akita);
      final asUtc = evaluateDaylight(DateTime.utc(2026, 1, 15, 6, 30), _akita);
    ...
    test('minutesToEvent has the right magnitude and sign (never negative)', () {
      // 06:30 with sunrise ~06:59 => ~29 min to the coming sunrise.
      final pre = evaluateDaylight(DateTime(2026, 1, 15, 6, 30), _akita);
      expect(pre.minutesToEvent, inInclusiveRange(27, 31),
      ...
      expect(post.minutesToEvent, inInclusiveRange(80, 84),
          reason: 'post-dusk minutes-since-sunset: ${post.minutesToEvent}');
    });
  });

  group('reference-instant selection (correction b)', () {
    test('pre-dawn instant names the COMING sunrise (same day, not prior day)', () {
      final d = evaluateDaylight(DateTime(2026, 1, 15, 6, 30), _akita);
      expect(d.phase, DaylightPhase.preDawn);
      _closeTo(d.eventHHMM, '06:59', 2, 'pre-dawn reference = coming sunrise');
  ...
  group('en/ja chip parity (same numbers, same structure, only language differs)', () {
    test('pre-dawn chip carries the identical sunrise time in both languages', () {
      final d = evaluateDaylight(DateTime(2026, 1, 15, 6, 30), _akita);
    ...
    test('post-dusk chip carries the identical sunset time in both languages', () {
      final d = evaluateDaylight(DateTime(2026, 1, 15, 18, 0), _akita);
      final en = PretripMessages.en.daylightChip(d);
      final ja = PretripMessages.ja.daylightChip(d);
    ...
    test('forLanguage resolves ja/en and falls back for unknown', () {
      final d = evaluateDaylight(DateTime(2026, 1, 15, 6, 30), _akita);
      expect(PretripMessages.forLanguage('ja_JP').daylightChip(d),
          PretripMessages.ja.daylightChip(d));
      expect(PretripMessages.forLanguage('fr').daylightChip(d),
          PretripMessages.en.daylightChip(d));
    });
  });

  group('no fabricated hazard (correction a)', () {
    test('daylight chip asserts no ice/refreeze for any phase, either language', () {
  ...
  group('polar phases', () {
    test('Tromso 2026-06-21 — POLAR DAY: full daylight, no low-light note', () {
      ...
      final b = const SnowAwarePretripAdvisor()
    ...
    test('Tromso 2025-12-21 — polar night WITH midday civil twilight (NOT deep dark)',
        () {
    ...
    test('80N 2025-12-21 — DEEP polar night (no civil twilight): all-day darkness',
        () {
      final d = evaluateDaylight(DateTime(2025, 12, 21, 12, 0), _highArctic);
      expect(d.phase, DaylightPhase.polarNight);
      expect(d.deepDark, isTrue,
  ...
  group('deep-dark vs twilight severity (correction: pick the DARKEST instant)', () {
    test('a deep pre-dawn instant is severity 2/deepDark; near sunrise is 1', () {
    ...
    test('a trip with a twilight pre-dawn departure AND a deep-dark post-dusk '
      ...
      expect(daylight.single, contains('past sunset'),
  ...
  group('advisor integration', () {
    ...
    test('daylight chip threads through the ja advisor (HER own language)', () {
      ...
      final ja = SnowAwarePretripAdvisor(messages: PretripMessages.ja)
          .brief(forecast: forecast, commute: commute, profile: _fastDriver);
      final en =
          advisor.brief(forecast: forecast, commute: commute, profile: _fastDriver);
      final jaLight = ja.chips.where((c) => c.contains('日の入り')).toList();
      expect(jaLight, hasLength(1), reason: 'ja daylight chip present');
      expect(RegExp(r'[぀-ヿ一-龯]').hasMatch(jaLight.single), isTrue,
    ...
    test('elevated trip: daylight note never restates the measured-visibility '
      ...
      for (final m in <PretripMessages>[PretripMessages.en, PretripMessages.ja]) {
        final b = SnowAwarePretripAdvisor(messages: m)
            .brief(forecast: forecast, commute: commute, profile: _fastDriver);
        final daylight = b.chips
        ...
        expect(daylight.single.contains('visibility'), isFalse,
            reason: 'daylight chip must not borrow the forecast visibility noun');
        expect(daylight.single.contains('視界'), isFalse);
      }
    });

    test('CLEAR trip ending after dark still gets the light note (post-dusk)', () {
      final forecast = _clearForecast(day, 14, 19);
      final commute = CommuteShape(
        plannedDuration: const Duration(hours: 1),
        routeIdentifiers: const ['r'],
      ...
      final b = advisor.brief(
          forecast: forecast, commute: commute, profile: _fastDriver);
      expect(b.verdict, PretripVerdict.clear);
      final light =
          b.chips.where((c) => c.contains('past sunset')).toList();
      expect(light, hasLength(1), reason: 'expected one post-dusk light note');
    ...
    test('PRE-DAWN departure is covered (names the coming sunrise)', () {
      ...
      final b = advisor.brief(
          forecast: forecast, commute: commute, profile: _fastDriver);
      final light =
          b.chips.where((c) => c.contains('before sunrise')).toList();
      expect(light, hasLength(1), reason: 'expected one pre-dawn light note');
      expect(light.single, contains('06:5')); // ~06:59 sunrise time
    });

    test('null geo => NO daylight chip (feature absent, fully backward compatible)',
        () {
      final forecast = _clearForecast(day, 14, 19);
      final commute = CommuteShape(
        plannedDuration: const Duration(hours: 1),
        routeIdentifiers: const ['r'],
        flexibility: CommuteFlexibility.discretionary,
        plannedDeparture: DateTime(2026, 1, 15, 18, 0), // after dark, but no geo
      );
      final b = advisor.brief(
          forecast: forecast, commute: commute, profile: _fastDriver);
      expect(
  ...
  group('TripGeo value equality', () {
    test('equal over all three fields; differs on any one', () {
      const a = TripGeo(latitude: 1, longitude: 2, utcOffset: Duration(hours: 9));
      const b = TripGeo(latitude: 1, longitude: 2, utcOffset: Duration(hours: 9));
      expect(a, equals(b));
      expect(a.hashCode, equals(b.hashCode));
      expect(
          a,
          isNot(equals(const TripGeo(
              latitude: 9, longitude: 2, utcOffset: Duration(hours: 9)))));
      expect(
          a,
          isNot(equals(const TripGeo(
              latitude: 1, longitude: 9, utcOffset: Duration(hours: 9)))));
      expect(
          a,
          isNot(equals(const TripGeo(
              latitude: 1, longitude: 2, utcOffset: Duration(hours: 1)))));
    });
  });
}
