...
const _enUnavailable =
    'Official-warning check unavailable — a warning may be '
...
void main() {
  ...
  group('the negative claim must be EARNED (0.5.3 default flip)', () {
    test('the zero-effort call no longer fabricates an all-clear: no publisher '
      ...
      expect(
        ...
        isNot(contains(_enNone)),
      );
      expect(
        areaConditionChips(r, PretripMessages.ja),
        isNot(contains(_jaNone)),
      );
    });
  ...
  group(
    'a hazard seen is a hazard real, even on partial data (0.5.3 reorder)',
    () {
      test('REGRESSION: a warning in hand renders even when the check was '
      ...
      test('a warning in hand with a complete check renders identically', () {
        ...
        expect(
          warningChip(partial, PretripMessages.ja),
  ...
  group(
    'THE RUNG COMES DOWN — and its release is the authority\'s, not ours',
    () {
      test('authority withdraws the warning => the chip FALLS from 大雪警報 to the '
        ...
        expect(
          warningChip(thursday, PretripMessages.ja),
          isNot(warningChip(sunday, PretripMessages.ja)),
        );
      });

      test('the fall is NOT on our release schedule: no parser update, no new '
  ...
  group('IT DOES NOT FLAP — there is no latch in either direction', () {
    test('pure: identical inputs render identically across repeated calls', () {
      ...
      final first = areaConditionChips(
        read(warning: null, checkComplete: true),
        PretripMessages.ja,
      );
      for (var i = 0; i < 50; i++) {
        expect(
      ...
      () {
        ...
        final rendered = [
          for (final w in sequence)
            warningChip(
              read(warning: w, checkComplete: true),
              PretripMessages.ja,
            ),
        ];
    ...
    test('order-independence: the chip for a given publisher state is the same '
      ...
      final downThenUp = [
        warningChip(
          read(warning: '大雪警報', checkComplete: true),
          PretripMessages.ja,
        ),
        warningChip(
          read(warning: null, checkComplete: true),
        ...
        for (final complete in [true, false]) {
          for (final m in [PretripMessages.en, PretripMessages.ja]) {
            final chips = areaConditionChips(
              read(warning: w, checkComplete: complete),
              m,
            );
            for (final chip in chips) {
              for (final banned in const [
                ...
                expect(
                  ...
                  reason:
                      'the subject is the AREA and the publisher\'s '
