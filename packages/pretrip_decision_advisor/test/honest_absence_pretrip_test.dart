...
WeatherForecast _emptyForecast(DateTime departure) =>
    WeatherForecast(issuedAt: departure, hourly: const []);
...
void main() {
  ...
  group('brief(): no forecast coverage STOPS, never fabricates clear', () {
    ...
    test(
      'the exception is a PretripDataAbsentException (one catch for all)',
      () {
        expect(
          () => _advisor.brief(
            forecast: _emptyForecast(departure),
  ...
  group('briefOrNull(): the branch-do-not-catch way forward', () {
    ...
    test('returns a real briefing when the window IS covered AND measured', () {
      ...
      // visibility, precipitation and road surface had never been measured.
      // The test written to certify the 0.5.2 honesty fix was asserting the
      // per-slot half of the same fabrication, one level down.
      //
      // The intent of the test is preserved exactly: a covered window returns
      // a real briefing. What changed is that the window must now be MEASURED
      // as well as covered, so the fixture states what it claims to know.
      ...
      expect(b, isNotNull);
      // The band is derived from a real slot whose deciding fields were all
      // measured — NOW it is safe to colour from.
    ...
    test('covered but UNMEASURED returns null — the 0.5.4 per-slot half '
      ...
      expect(
        _advisor.briefOrNull(
          forecast: coveredButBlind,
          commute: _commute(departure),
          profile: _profile,
        ),
        isNull,
        reason:
            'a temperature alone decides nothing about visibility, '
            'precipitation or the road surface; briefOrNull must say '
            '"we do not know", never report a clear window',
      );
    });
  });
  ...
  group(
    'AreaConditionRead: uncovered window STOPS, never fabricates clear',
    () {
