...
///    [DigitrafficVisibilityProvider.maxDistanceKm].
/// 2. GET `/api/weather/v1/stations/{id}/data` (~2 KB) → the `NÄKYVYYS_M`
///    sensor (or `NÄKYVYYS_KM` × 1000), accepted only when `measuredTime`
///    is within [DigitrafficVisibilityProvider.maxObservationAge].
///
/// Honesty rules, binding:
/// - An observation is a NOW measurement at a station up to
///   [DigitrafficVisibilityProvider.maxDistanceKm] away — the caller applies
///   it to the current (departure) hour only, never projects it forward into
///   forecast hours, and surfaces the station distance.
...
///
/// SAFETY CONTRACT (verbatim — see README.md, binding on every consumer):
...
/// Fintraffic-required attribution; surface it at the consumer-facing layer.
const String kDigitrafficVisibilityAttributionString =
