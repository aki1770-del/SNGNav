...
class VehicleConditionSignals extends Equatable {
  ...
  ///    `null` (absent). It is **never clamped**: clamping to the maximum would
  ///    assert *perfect grip* on a road nobody measured.
  ...
  /// The road-friction leaf in VSS **percent** (0..100), or `null` when the
  /// producer did not give us a usable reading.
  ///
  /// A finite value **outside** `0..100` is out of spec: the producer is broken,
  /// and we say so by returning `null` (absent) rather than clamping it into
  /// range. The previous code clamped, and that is how a broken ESC's classic
  /// `255` "no reading" sentinel became `(255/100).clamp(0,1) == 1.0` — a
  /// *positively measured* full grip, manufactured out of a dead sensor and
  /// handed to every caller reading [roadFriction].
  ///
  /// Clamping an out-of-range reading to the maximum asserts perfect grip on a
  /// road nobody measured, which is the exact failure this seam exists to
  /// prevent — the same rule `kuksa_dart_sdk`'s `RoadFriction` enforces on the
  /// producer side, where an out-of-spec value is `RoadGrip.unknown`.
