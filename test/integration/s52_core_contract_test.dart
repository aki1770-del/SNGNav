...
void main() {
  group('S52 _core contract validation', () {
    test(
      'navigation_safety_core symbols stay mirrored by the parent package barrel',
      () {
        ...
        expect(
          nav_full.AlertSeverity.warning,
          same(nav_core.AlertSeverity.warning),
        );
        expect(
    ...
    test(
      'routing_bloc_core symbols stay mirrored by the parent package barrel',
      () {
    ...
    test(
      'map_viewport_bloc_core symbols stay mirrored by the parent package barrel',
      () {
        final map_core.CameraMode modeFromFull = map_full.CameraMode.follow;
        final map_full.MapLayerType layerFromCore =
            map_core.MapLayerType.weather;

        expect(modeFromFull, map_core.CameraMode.follow);
        expect(layerFromCore.zIndex, map_full.MapLayerZ.weather);
        expect(
          map_full.MapLayerZ.of(layerFromCore),
          map_core.MapLayerZ.weather,
        );
    ...
    test(
      'offline_tiles_core symbols stay mirrored by the parent package barrel',
      () {
        final tiles_core.CoverageTier tierFromFull =
            tiles_full.CoverageTier.t1Corridor;
        final tiles_full.TileCacheConfig configFromCore =
            const tiles_core.TileCacheConfig();
        final tiles_core.TileSourceType sourceFromFull =
            tiles_full.TileSourceType.mbtiles;

        expect(tierFromFull.autoCache, isTrue);
        expect(configFromCore.minZoomFor(tiles_full.CoverageTier.t2Metro), 9);
        expect(
          configFromCore.maxZoomFor(tiles_core.CoverageTier.t4National),
          10,
        );
