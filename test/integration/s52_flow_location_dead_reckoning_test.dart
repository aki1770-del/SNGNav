...
void main() {
  group('S52 Flow 3: GPS -> dead reckoning -> location', () {
    ...
    test(
      'Kalman GPS recovery improves confidence and clears DR flag',
      () async {
        final gps = _MockGpsProvider();
        final provider = DeadReckoningProvider(
          inner: gps,
        ...
        expect(
          bloc.state.confidenceRadius,
          lessThanOrEqualTo(gpsConfidence + 5),
        );
        expect(
    ...
    test(
      'extended Kalman-only loss exceeds safety cap and stops DR emissions',
      () async {
        final gps = _MockGpsProvider();
        final provider = DeadReckoningProvider(
          inner: gps,
        ...
        expect(
          ...
          isFalse,
          reason:
              'Kalman DR should stop immediately when uncertainty '
        ...
        expect(
          ...
          reason:
              'DR cap trip must emit an explicit error on the stream; '
