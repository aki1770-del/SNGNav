...
void main() {
  group('WinterKnowledge.fromJsonString', () {
    ...
    test('parses baked cards keyed by surface state', () {
      ...
      expect(
        wk.cardFor(RoadSurfaceState.blackIce)?.guidance,
    ...
    test('every baked card is grounded (action vocabulary, not a refusal)', () {
      ...
      for (final state in wk.states) {
        ...
        expect(
          refusal.hasMatch(g),
          isFalse,
          reason: '$state card is a refusal',
        );
        expect(
          action.hasMatch(g),
          isTrue,
  ...
  group('Japanese guidance (guidance_ja)', () {
    ...
    test(
      'guidanceForLanguage(ja) returns Japanese, else falls back to English',
      () {
        const both = WinterCard(
          state: 'blackIce',
          guidance: 'Reduce speed.',
          guidanceJa: '速度を落とす。',
        );
    ...
    test('cardFor(lang: ja) resolves the Japanese text through the loader', () {
      ...
      expect(
        wk.cardFor(RoadSurfaceState.blackIce, lang: 'ja')?.guidance,
        contains('車間距離'),
      );
    ...
    test(
      'shipped asset: every Japanese card is grounded and keeps its figures',
      () {
        final wk = WinterKnowledge.fromJsonString(file.readAsStringSync());
        ...
        for (final state in wk.states) {
          ...
          jaCount++;
          expect(
            jaRefusal.hasMatch(ja),
            isFalse,
            reason: '$state JA card is a refusal',
          );
          expect(
            jaAction.hasMatch(ja),
            isTrue,
            reason: '$state JA card lacks action vocabulary',
          );
          expect(
            ja.contains('5〜6秒') || ja.contains('5～6秒'),
            isTrue,
            reason:
                '$state JA card lost the 5–6 second following-distance figure',
          );
        }
