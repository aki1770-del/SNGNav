...
/// (約2).
const String _csvThreeBridges = 'way_id,lat,lon,bearing_deg\n'
    '1,39.72500,140.10000,0\n'
...
const String _csvFarBridge = 'way_id,lat,lon,bearing_deg\n'
...
WeatherForecast _cannedLive(double temp) {
  ...
  return WeatherForecast(
    ...
    hourly: [
      ...
      HourlyForecast(
          hour: base.add(const Duration(hours: 1)), tempCelsius: temp),
    ],
  );
}
...
void main() {
  ...
  Future<void> setPlaceJa(
      WidgetTester tester, double lat, double lon, String label) async {
  ...
  testWidgets(
    ...
    expect(
      ...
      findsOneWidget,
      reason: 'the pre-trip surface never shows the map tile credit, so the '
  ...
  testWidgets(
      ...
      'plus off-season: quiet)', (tester) async {
    await pumpBridge(
      tester,
      routeBridges: () => resolvePretripRouteBridges(
        fetchRouteShape: () async => _route,
  ...
  testWidgets(
      ...
      'fires exactly ONE fresh resolve for the new epoch', (tester) async {
    var calls = 0;
    ...
    expect(find.byKey(_cautionKey), findsNothing,
        reason: 'the old count described the route to the PRIOR destination — '
            'it must clear before the new resolve lands');
    expect(find.textContaining('経路上に橋が約'), findsNothing);
    expect(calls, 2,
        reason: 'exactly ONE fresh resolve fires for the new epoch');
    await flush(tester);
    expect(calls, 2, reason: 'and no further resolve fires — one per epoch, '
        'never a poll');
  ...
  testWidgets(
      ...
      'resolves (no route ⇒ nothing to resolve)', (tester) async {
    var calls = 0;
    ...
    expect(find.textContaining('経路上に橋が約'), findsNothing);
    expect(calls, 1,
        reason: 'a clear fires ZERO new resolves — cleared, and the epoch '
            'ends there');
  });

  testWidgets(
      '(i) destination change MID-FLIGHT: the late OLD-epoch result is '
    ...
    await pumpBridge(
      ...
      routeBridges: () async {
        ...
        return resolvePretripRouteBridges(
          fetchRouteShape: () async => _route,
          loadBridgeCsv: () async =>
              n == 0 ? _csvTwoBridges : _csvThreeBridges,
        );
      },
      gateNow: DateTime(2026, 1, 15, 7),
    );
    ...
    await flush(tester);
    expect(find.byKey(_cautionKey), findsNothing,
        reason: 'the late result describes a route SHE is not taking — '
    ...
    gates[1].complete();
    await flush(tester);
    expect(find.textContaining('この先、秋田県内の経路上に橋が約3か所あります'),
        findsOneWidget,
        reason: 'the new epoch\'s result (約3) must land — and NOT the stale '
            '約2');
  ...
  testWidgets(
      ...
      'change must NOT feed the new destination\'s cold gate', (tester) async {
    ...
    await flush(tester);
    expect(find.byKey(_cautionKey), findsNothing,
        reason: 'the freezing forecast describes エリアA, which SHE is no '
  ...
  testWidgets(
      ...
      (tester) async {
    final requests = <({double oLat, double oLon, double dLat, double dLon})>[];
    await pumpBridge(
      tester,
      ...
      routeShapeFetch: ({
        ...
        requests.add(
            (oLat: originLat, oLon: originLon, dLat: destLat, dLon: destLon));
        return _route;
      },
  ...
  testWidgets(
      ...
      'set fires exactly ONE fetch, for the just-set place', (tester) async {
    ...
    expect(requests, hasLength(1),
        reason: 'the first set fires exactly ONE fetch (no double-fetch from '
